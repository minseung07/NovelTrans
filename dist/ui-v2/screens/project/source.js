...
const LIMIT = 10;
const LANG_LABELS = { ja: "일본어", ko: "한국어", unknown: "알 수 없음" };
...
function previewLines(episode) {
    const lines = [`${episode.episodeNo}화  ${episode.title}`, ""];
    if (episode.foreword) {
...
export function renderSource(project, selected, width) {
