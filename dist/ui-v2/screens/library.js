...
function listCard(model, width, rows) {
...
function problemCard(model, width) {
    if (model.library.problemProjects.length === 0) {
        return [];
    }
    return box("확인 필요", model.library.problemProjects.slice(0, 4).map((project) => `${project.title}  ${project.shelfStatusLabel}  ${project.nextActionLabel}`), width);
}
export function renderLibrary(model, width, rows) {
