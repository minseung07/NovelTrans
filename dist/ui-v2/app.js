...
function renderOverlay(model, width) {
...
function handleGlossaryKey(token, dispatch) {
    const ops = { c: { type: "glossary-op", op: "confirm" }, l: { type: "glossary-op", op: "lock" }, f: { type: "glossary-op", op: "forbid" }, d: { type: "glossary-op", op: "discard" }, e: { type: "glossary-edit-open" }, a: { type: "glossary-filter" } };
    if (token === "up" || token === "k") {
...
function handleQaKey(token, dispatch) {
    const ops = { i: { type: "qa-op", op: "ignore" }, r: { type: "qa-op", op: "recheck" }, t: { type: "qa-op", op: "retranslate" }, g: { type: "qa-jump-glossary" } };
    if (token === "up" || token === "k") {
        dispatch({ type: "move", delta: -1 });
        return true;
    }
...
function handleTranslateKey(token, dispatch) {
    if (token === "t") {
...
function handleSourceKey(token, dispatch) {
    if (token === "up" || token === "k") {
        dispatch({ type: "move", delta: -1 });
        return true;
    }
...
function handleExportKey(token, dispatch) {
    const toggles = { t: { type: "export-toggle", what: "txt" }, e: { type: "export-toggle", what: "epub" }, p: { type: "export-toggle", what: "appendix" }, v: { type: "export-toggle", what: "vertical" }, a: { type: "export-toggle", what: "afterword" }, g: { type: "export-generate" } };
    if (toggles[token]) {
...
function handleOverlayKey(model, event, token, dispatch) {
    const overlay = model.overlay;
    ...
    if (overlay.kind === "settings") {
        const ops = { b: { type: "settings-op", op: "cycle-backend" }, m: { type: "settings-op", op: "cycle-model" }, g: { type: "settings-op", op: "cycle-strictness" }, "+": { type: "settings-op", op: "inc-concurrency" }, "=": { type: "settings-op", op: "inc-concurrency" }, "-": { type: "settings-op", op: "dec-concurrency" }, t: { type: "settings-op", op: "toggle-txt" }, e: { type: "settings-op", op: "toggle-epub" } };
...
export function onKey(model, event, ctx) {
...
export async function runUiV2(options) {
    ...
    await runProgram({ init: initModel(options.config, library), update, view, onKey, runEffect }, { terminal });
}
