...
export function createEmptyGlossary() {
    return {
        version: 1,
        entries: [],
        conflicts: [],
...
export function extractGlossaryCandidates(episodes, existing = createEmptyGlossary()) {
...
export function confirmGlossaryTerm(data, source, target, lock = false) {
    const now = nowIso();
...
export function addForbiddenTarget(data, source, forbiddenTarget) {
    const now = nowIso();
    const entries = [...data.entries];
...
export function deprecateGlossaryTerm(data, source) {
    const now = nowIso();
    const entries = [...data.entries];
    const entry = entries.find((candidate) => candidate.source === source);
    if (!entry) {
...
export function addTargetCandidate(data, source, target, episodeId) {
    const now = nowIso();
    const entries = [...data.entries];
    let entry = entries.find((candidate) => candidate.source === source);
    if (!entry) {
...
export function mergeTranslationGlossaryCandidates(data, candidates, episodeId) {
    let next = data;
...
export function detectGlossaryConflicts(entries) {
...
function confirmedTargetCandidates(entry, target) {
...
export function buildGlossaryContext(entries, strictness) {
...
function collectCandidates(episodes) {
...
function bumpCandidate(candidates, source, episode, quoted) {
...
function mergeTargetCandidate(candidates, target, episodeId) {
    if (!target) {
        return candidates;
    }
...
function parseTranslationGlossaryCandidate(candidate) {
