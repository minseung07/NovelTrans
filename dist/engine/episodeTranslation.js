...
export async function translateEpisodeParts(options) {
    ...
    for (const [index, chunk] of bodyChunks.entries()) {
        ...
        bodyResults.push(await options.adapter.translateEpisode({
            ...baseInput,
            episode
        }));
    }
...
function mergeBodyResults(episode, results) {
    ...
    return {
        ...
        summary: results.map((result) => result.summary).filter((summary) => Boolean(summary)).join("\n") || first.summary,
...
function buildChunkEpisode(episode, body, chunkNo, chunkCount) {
...
function buildSegmentEpisode(episode, segment, text) {
    const { foreword: _foreword, afterword: _afterword, ...baseEpisode } = episode;
    return {
        ...baseEpisode,
