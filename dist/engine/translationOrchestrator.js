...
import { listEpisodes, loadGlossary, saveGlossary, saveProjectMetadata, saveQAIssues, saveTranslation, readAllQAIssues, writeQualityReport } from "../storage/projectStore.js";
...
export async function translateProjectQueue(options) {
...
async function runWorker(queue, options, stateStore, glossaryUpdater) {
    let completed = 0;
    let failed = 0;
    const qaIssues = [];
...
function shouldQueue(state, mode) {
    if (!state) {
        return true;
    }
...
async function collectAndPersistQualityReport(projectDir, currentIssues) {
