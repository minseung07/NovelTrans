...
const defaultQAOptions = {
    japaneseRemaining: true,
    numberMismatch: true,
    lengthRatio: true,
    glossary: true
};
export function runQA(episode, result, glossary, options = defaultQAOptions) {
    const issues = [];
    const add = (issue) => {
        issues.push({
            id: newId("qa"),
...
function firstParagraph(paragraphs, predicate) {
...
function translationVariantsFor(entry, bodyKo) {
