...
export function renderBookshelfScreen(model, selectedIndex = 0, width) {
    const body = [];
    ...
    body.push(...box("이어하기", continueProject
        ? [
        ...
        : ["아직 프로젝트가 없습니다.", "[N] 새 작품 가져오기"], width), "");
    body.push(...box("최근 프로젝트", model.recentProjects.length > 0
        ...
        : [`프로젝트 폴더: ${model.projectRoot}`, "[N]으로 원문 경로, URL, 붙여넣기를 시작하세요."], width));
    if (model.problemProjects.length > 0) {
        body.push("", ...box("확인 필요", model.problemProjects
            .slice(0, 4)
            .map((project) => `${project.title}  ${project.shelfStatusLabel}  ${project.nextActionLabel}`), width));
    }
