...
export function renderReviewDeskScreen(model, selectedIndex = 0, width) {
    ...
    return renderScreen("검수 작업대", model.overview.metadata.name, body, "[Enter] 열기   [T] 선택 재번역   [A] 전체 재번역   [F] 같은 유형   [M] 무시   [C] 재검사   [G] 용어   [B] 뒤로", { width });
}
function formatIssue(issue, selected) {
...
function reviewFocusLines(model) {
