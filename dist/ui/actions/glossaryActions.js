...
export async function confirmSelectedGlossaryTerm(projectDir, model, selectedIndex, target, lock = false, filter = "all", deferredEntryIds = []) {
...
export async function forbidSelectedGlossaryTarget(projectDir, model, selectedIndex, target, filter = "all", deferredEntryIds = []) {
    const selected = selectedGlossaryQueueItem(model, selectedIndex, filter, deferredEntryIds);
    if (!selected) {
        return "선택된 용어가 없습니다.";
    }
...
export async function discardSelectedGlossaryTerm(projectDir, model, selectedIndex, filter = "all", deferredEntryIds = []) {
    const selected = selectedGlossaryQueueItem(model, selectedIndex, filter, deferredEntryIds);
    if (!selected) {
        return "선택된 용어가 없습니다.";
    }
...
export function suggestedGlossaryTarget(model, selectedIndex, filter = "all", deferredEntryIds = []) {
    const selected = selectedGlossaryQueueItem(model, selectedIndex, filter, deferredEntryIds);
    if (!selected) {
        return null;
    }
...
export async function exportGlossaryJson(projectDir) {
    const glossary = await loadGlossary(projectDir);
...
export function relatedTermsForEpisode(model, episodeTitle) {
    const episode = (episodeTitle ? model.episodes.find((candidate) => candidate.title === episodeTitle) : null) ??
...
export function relatedTermsForIssue(model, selectedIssueIndex) {
...
function formatRelatedTerms(model, episodeId, episodeTitle, sourceText) {
