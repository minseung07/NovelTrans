...
export function failedReviewRetranslationSnapshot(error, title) {
    return {
        status: "failed",
        queued: 1,
        completed: 0,
...
export function buildReviewRetranslationQueue(model, selectedIssueIndex, scope) {
    const selected = selectedOpenIssue(model, selectedIssueIndex);
    if (!selected) {
        return [];
    }
    const issues = scope === "selected"
        ? [selected]
...
function enqueueReviewRetranslationItems(state, items) {
