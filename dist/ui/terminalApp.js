...
import { backFromTerminalSpace, createInitialTerminalState, moveTerminalSelection } from "./terminalState.js";
...
import { confirmSelectedGlossaryTerm, discardSelectedGlossaryTerm, forbidSelectedGlossaryTarget, relatedTermsForIssue, suggestedGlossaryTarget } from "./actions/glossaryActions.js";
import { clearCoverImagePath, formatExportPreview, generateAllExports, generateConfiguredExports, openOutputFolder, setCoverImagePath, toggleAfterword, toggleGlossaryAppendix, toggleOutputFormat, toggleVerticalWriting } from "./actions/exportActions.js";
...
import { primaryStudioKeyIntent } from "./studioKeyIntents.js";
import { completeTask, createRunningTask, failTask, isTaskRunning } from "./taskStatus.js";
import { buildReviewRetranslationQueue, createReviewRetranslationTask, failedReviewRetranslationSnapshot } from "./reviewRetranslationTask.js";
import { hideReviewDeskEpisodes } from "./reviewDeskModel.js";
import { createUndoAction, hideUndoHint, scopedUndoAction, visibleUndoHint } from "./undoState.js";
export async function runTerminalStudio(options) {
...
class TerminalStudioApp {
    ...
    reviewRetranslationSnapshot = null;
    ...
    async editSelectedAdvancedSetting() {
    ...
    async handleSettingsPickerKey(key) {
        const item = this.selectedAdvancedSettingsItem();
    ...
    async applyAdvancedSettingOption(item, option) {
    ...
    async applyModelToOpenProject(backend, model) {
    ...
    async handleGlossaryKey(key) {
        ...
        if (key === "\r") {
            ...
            if (target) {
                this.state.message = await confirmSelectedGlossaryTerm(this.state.projectDir, model, this.state.selectedTermIndex, target, false, this.state.glossaryFilter, this.state.deferredGlossaryEntryIds);
            }
            return;
        }
        ...
        if (key.toLowerCase() === "l") {
            ...
            if (target) {
                this.state.message = await confirmSelectedGlossaryTerm(this.state.projectDir, model, this.state.selectedTermIndex, target, true, this.state.glossaryFilter, this.state.deferredGlossaryEntryIds);
            }
            return;
        }
        if (key.toLowerCase() === "f") {
            ...
            if (target) {
                this.state.message = await forbidSelectedGlossaryTarget(this.state.projectDir, model, this.state.selectedTermIndex, target, this.state.glossaryFilter, this.state.deferredGlossaryEntryIds);
            }
            return;
        }
        ...
        if (key.toLowerCase() === "d") {
            ...
            this.pendingConfirmation = {
                ...
                onConfirm: async () => {
                    this.state.message = await discardSelectedGlossaryTerm(projectDir, model, selectedIndex, filter, deferredEntryIds);
    ...
    async handleFailureRecoveryKey(key) {
        if (!this.state.projectDir) {
            return;
        }
    ...
    async applyPaletteCommand(commandId) {
        this.state.paletteQuery = "";
    ...
    async applyStudioKeyIntent(intent) {
    ...
    async openSelectedBookshelfProject(space) {
    ...
    async openSelectedBookshelfReview() {
        const project = selectedBookshelfProject(await loadBookshelfModel(this.projectRoot), this.state.selectedProjectIndex);
        if (!project) {
    ...
    async handleConfirmationKey(key) {
    ...
    requestQuitConfirmationIfActive() {
    ...
    startExportTask(projectDir, mode) {
    ...
    startSkipFailedExportTask(projectDir) {
        this.startBackgroundTask({
    ...
    async restoreGlossaryTerm(source, status, locked) {
        if (!this.state.projectDir) {
    ...
    async restoreIgnoredIssue(issueId) {
        if (!this.state.projectDir) {
            return "열린 프로젝트가 없습니다.";
        }
    ...
    async currentSelectionMaxIndex() {
        if (this.state.space === "bookshelf") {
    ...
    async startTranslationSession(mode) {
        if (!this.state.projectDir) {
            return;
        }
        if (isTaskRunning(this.activeTask)) {
    ...
    async confirmReviewRetranslation(model, scope) {
        if (!this.state.projectDir) {
            return;
        }
    ...
    startReviewRetranslation(projectDir, model, selectedIssueIndex, adapter, scope) {
...
function reviewRetranslationSummary(scope, queueLength, issueType) {
    if (scope === "all-open") {
...
function isGlossaryStrictness(value) {
    return value === "low" || value === "medium" || value === "high" || value === "strict";
}
function toggleQaSetting(config, itemId) {
...
function toggleEpubSetting(config, itemId) {
