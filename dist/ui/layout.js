...
export function normalizeScreenWidth(width) {
    if (!width || !Number.isFinite(width)) {
        return screenWidth;
    }
    return Math.max(minScreenWidth, Math.min(maxScreenWidth, Math.floor(width)));
}
export function renderScreen(title, subtitle, body, footer, options = {}) {
...
export function box(title, lines, width = screenWidth) {
...
function normalizeBoxWidth(width) {
    if (!width || !Number.isFinite(width)) {
        return screenWidth;
    }
    return Math.max(20, Math.min(maxScreenWidth, Math.floor(width)));
}
export function columns(leftTitle, leftLines, rightTitle, rightLines, width = screenWidth) {
    const gap = 3;
...
export function table(rows, labelWidth = 14) {
    return rows.map(([label, value]) => `${padRight(label, labelWidth)} ${value}`);
}
export function bullet(lines) {
...
export function fitToViewport(screen, viewport = {}) {
