...
export class WebImportService {
    ...
    async importProject(preview, options, onProgress) {
...
export function composeWebSourceText(episodes) {
    return episodes
        .map((episode) => {
...
export function webImportConsentMessage(work, selection, count) {
    const siteLabel = work.site === "syosetu" ? "소설가가 되자" : "카쿠요무";
    const caution = work.site === "syosetu"
...
async function attachWebEpisodeMetadata(projectDir, work, webEpisodes) {
    const episodes = await listEpisodes(projectDir);
    const webByIndex = new Map();
