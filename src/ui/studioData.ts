...
export async function loadBookshelfModel(projectRoot: string): Promise<BookshelfModel> {
  ...
  const projects: BookshelfProject[] = (await Promise.all(overviews.map((overview) => buildBookshelfProject(overview)))).sort((left, right) =>
...
export async function loadProjectUiModel(projectDir: string): Promise<ProjectUiModel> {
