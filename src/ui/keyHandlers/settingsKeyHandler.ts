...
import type { SettingsViewMode } from "../types.js";
import {
  adjustConcurrency,
  cycleCodexCliModel,
  cycleDefaultBackend,
...
export type SettingsKeyHandlerInput = {
  key: string;
...
export type SettingsKeyHandlerResult = {
  config: NovelTransConfig;
  message: string | null;
};

export async function handleSettingsKey(input: SettingsKeyHandlerInput): Promise<SettingsKeyHandlerResult> {
  const mode = input.mode ?? "basic";
  const preset = Number(input.key);
  if (mode === "basic" && preset >= 1 && preset <= 6) {
    const config = await saveRecipePreset(input.config, preset as RecipePresetId, input.configDir);
