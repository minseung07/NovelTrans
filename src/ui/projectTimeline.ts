...
import type { ProjectTimelineItem } from "./types.js";

const timelineLimit = 6;

export function buildProjectTimeline(events: LogEntry[]): ProjectTimelineItem[] {
  return events
    .slice()
...
function timelineLabel(event: LogEntry): string {
...
function localizedTimelineMessage(event: LogEntry): string {
...
function replaceCount(message: string, pattern: RegExp, suffix: string): string {
  const match = message.match(pattern);
  return match ? `${match[1]}${suffix}` : message;
}

function replaceEpisodeSuffix(message: string, suffix: string, replacement: string): string {
  return message.endsWith(suffix) ? `${message.slice(0, -suffix.length)}${replacement}` : message;
}

function timelineSeverity(event: LogEntry): ProjectTimelineItem["severity"] {
