...
export function renderExportRoomScreen(model: ProjectUiModel, width?: number): string {
  ...
  return renderScreen(
    ...
    body,
    "[1/2] 형식   [A/W/V/C] 옵션   [P] 확인   [Enter] 생성   [B] 뒤로",
    { width }
  );
}

function checked(value: boolean, label: string): string {
