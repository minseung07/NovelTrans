...
export async function openFile(path: string, options: OpenFileOptions = {}): Promise<OpenFileResult> {
  ...
  try {
    ...
    return {
      opened: false,
      command: [executable, ...args].join(" "),
      message: `열지 못했습니다: ${path} (${(error as Error).message})`
    };
  }
  return {
    opened: true,
...
function defaultOpenCommand(): string | undefined {
  if (platform() === "darwin") {
    return "open";
  }
  if (platform() === "win32") {
...
function runAndWait(executable: string, args: string[]): Promise<void> {
...
function spawnDetached(executable: string, args: string[]): Promise<Error | null> {
