...
import type { AdvancedSettingsItem, AdvancedSettingsOption, AdvancedSettingsSection, SettingsViewMode } from "./types.js";
...
export type ApiKeyState = "environment" | "stored" | "missing" | "unreadable" | "unknown";

export type SettingsConnectionState = {
  openAICompatibleApiKey: ApiKeyState;
};
...
export function settingsTitle(mode: SettingsViewMode): string {
  return mode === "advanced" ? "세부 설정" : "번역 설정";
}

export function settingsSubtitle(mode: SettingsViewMode): string {
  return mode === "advanced" ? "새 작품 기본값 · 직접 조정" : "새 작품 기본값";
}

export function settingsFooter(mode: SettingsViewMode): string {
  if (mode === "advanced") {
...
export function settingsPickerFooter(item: AdvancedSettingsItem): string {
  if (item.kind === "number") {
...
export function buildSettingsSections(config: NovelTransConfig, mode: SettingsViewMode, connectionState?: SettingsConnectionState): SettingsSection[] {
  if (mode === "advanced") {
...
function buildBasicSettingsSections(config: NovelTransConfig): SettingsSection[] {
  return [
    {
      title: "현재 레시피",
      lines: [
...
function buildAdvancedSettingsSections(config: NovelTransConfig, connectionState?: SettingsConnectionState): SettingsSection[] {
...
export function buildAdvancedSettingsForm(config: NovelTransConfig, connectionState?: SettingsConnectionState): AdvancedSettingsSection[] {
  return [
    {
      id: "engine",
      title: "엔진",
...
export function advancedSettingsOptions(item: AdvancedSettingsItem): AdvancedSettingsOption[] {
  if (item.kind === "number") {
...
export function selectedAdvancedSettingsOptionIndex(item: AdvancedSettingsItem): number {
...
function numberOptions(item: AdvancedSettingsItem): AdvancedSettingsOption[] {
  const min = item.min ?? 0;
  const max = item.max ?? min;
  const step = item.step ?? 1;
  const options: AdvancedSettingsOption[] = [];
...
function nearestNumberOptionIndex(options: AdvancedSettingsOption[], rawValue: string): number {
...
export function backendLabel(backend: NovelTransConfig["defaultBackend"]): string {
...
export function strictnessLabel(strictness: NovelTransConfig["glossaryStrictness"]): string {
...
export function outputLabel(formats: NovelTransConfig["outputFormats"]): string {
  return formats.map((format) => format.toUpperCase()).join(", ");
}

function modelLabel(config: NovelTransConfig): string {
...
function apiKeyLabel(state: ApiKeyState): string {
