...
import type { ApiKeyState } from "./settingsModel.js";
...
export type TerminalRendererInput = {
  state: TerminalState;
...
export async function renderTerminalScreen(input: TerminalRendererInput): Promise<string> {
  const { state } = input;
...
function openAICompatibleApiKeyState(configDir?: string): ApiKeyState {
