...
import type { WebImportPreview, WebWorkIndex } from "../webImport/types.js";
import { writeWebImportProgress, writeWebWorkLoadingStatus } from "./webImportProgress.js";

export type ImportDropInFlowOptions = {
...
export type ImportDropInFlowResult = {
  config: NovelTransConfig;
...
export async function runImportDropInFlow(options: ImportDropInFlowOptions): Promise<ImportDropInFlowResult> {
    ...
    if (source.kind === "web") {
      try {
        ...
        return {
          ...
          projectDir: null,
          targetSpace: null,
          message: `웹 원문 가져오기 실패: ${(error as Error).message}`
        };
      }
    }
...
async function readImportAnalysisChoice(
  reader: TerminalLineReader,
  output: WriteStream,
  analysis: ReturnType<typeof analyzeSource>,
  config: NovelTransConfig,
  configDir?: string
): Promise<{ config: NovelTransConfig; intent: ReturnType<typeof parseImportAnalysisChoice> }> {
...
type ImportSourceSelection =
  | { kind: "text"; sourceText: string; pasted: boolean; sourcePath: string | null }
  | { kind: "web"; url: string };

async function readSourceWithRetry(
  reader: TerminalLineReader,
  output: WriteStream
): Promise<ImportSourceSelection | null> {
  while (true) {
    ...
    try {
      ...
      output.write(`\n원문을 읽을 수 없습니다: ${(error as Error).message}\n다른 경로를 붙여넣거나, :paste 를 쓰거나, Enter로 취소하세요.\n> `);
    }
  }
}

async function readWebImportPreview(
  reader: TerminalLineReader,
  output: WriteStream,
  service: WebImportService,
  work: WebWorkIndex
): Promise<WebImportPreview | null> {
...
async function promptImportAnalysisChoice(reader: TerminalLineReader, output: WriteStream): Promise<"start" | "recipe" | "glossary" | "cancel"> {
  while (true) {
...
async function readPastedSource(reader: TerminalLineReader, output: WriteStream): Promise<string> {
  const lines: string[] = [];
...
function cancelled(config: NovelTransConfig): ImportDropInFlowResult {
  return {
    config,
    projectDir: null,
    targetSpace: null,
...
function projectOptionsFromConfig(config: NovelTransConfig, projectRoot: string) {
  return {
    projectRoot,
