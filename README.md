...
GitHub Actions publishes to npm from the `Release` workflow when a GitHub release is published.
...
The release tag must match `package.json` exactly as `v<version>`, for example `v2.1.5`.
