...
- 원문 분석, 프로젝트 생성, 번역, 내보내기 단계가 분리된 진행 화면
...
- `새 원문 번역`: 프로젝트 제목과 원문을 넣고 번역 준비 요약에서 바로 시작
...
새 번역 작업은 먼저 프로젝트 제목을 받고, 이어서 원문을 받습니다. 그 다음 아래처럼 짧은 요약을 보여주고, 사용자는 바로 시작하거나 필요한 항목만 바꿀 수 있습니다.
...
시작을 확정하면 바로 준비 상태 화면으로 바뀝니다. 큰 파일 분석, URL 메타데이터 조회, 허용된 본문 가져오기, 프로젝트 생성처럼 시간이 걸리는 단계도 화면에 현재 단계와 진행 수를 표시합니다. 번역 중에는 전체 화면을 매초 지우지 않고 상태 블록만 갱신합니다.
